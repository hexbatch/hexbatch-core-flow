CREATE TRIGGER trigger_after_flow_project_users_update_20210708235839
    AFTER UPDATE ON flow_project_users
    FOR EACH ROW
BEGIN
    ...
    IF NEW.can_read = 1 THEN
        ...
        IF (reader_user_guid IS NULL OR project_guid IS NULL) THEN
            SET msg := CONCAT('Flow Project Users update cannot find the user and project guids!, id [', NEW.id,
    ...
    IF NEW.can_read = 0 AND OLD.can_read = 1 AND
       NEW.flow_user_id = OLD.flow_user_id AND
       NEW.flow_project_id = OLD.flow_project_id
    THEN
        SELECT
            HEX(u.flow_user_guid)
        INTO
