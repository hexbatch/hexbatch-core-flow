...
function flow_tag_show_editor(tag){
    ...
    editing_div.find('.flow-attribute-show-edit-on-click').click(function() {
        let guid = $(this).data('attribute_guid');
        if (guid) {
            //find attribute
            let found_attribute = null;
            for(let attribute_name in tag.attributes) {
                let attribute = tag.attributes[attribute_name];
                if (attribute.flow_tag_attribute_guid === guid) {found_attribute = attribute; break;}
            }
            if (!found_attribute) {console.warn("could not find attribute for guid ",guid); return;}
    ...
    editing_div.find('button.flow-create-attribute-action').click(function() {
            ...
            flow_attribute_show_editor(tag,null,
                function(new_attribute) {
                    //see if attribute is in the list, if so replace, else add
                    let found_attribute_name = null;
                    for(let attribute_name in tag.attributes) {

                        if (new_attribute.tag_attribute_name === attribute_name) {
                            found_attribute_name = attribute_name; break;
                        }
                    }

                    if (found_attribute_name) {
                        tag.attributes[found_attribute_name] = new_attribute;
                    } else {
